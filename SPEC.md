...
### 5.5 CodeMirror 语法高亮规格

**MVP 实现方式**：`StreamLanguage.define()` + 正则 tokenizer（[`src/lib/editor/policy-language.ts`](src/lib/editor/policy-language.ts)），含可选错误区间装饰 `buildErrorHighlightExtensions`。V2 可升级为 Lezer（见 §10）。
...
表中「版本」列为选型说明；**实际依赖版本以 `package.json` 与 lockfile 为准**。
...
│   │   │
│   │   ├── stores/
│   │   │   └── playground-store.ts          # Zustand store（§4.1 + 可视化构建状态）
│   │   │
│   │   └── hooks/
...
│
├── tailwind.config.js                       # Tailwind 配置（含 DESIGN.md §2 色彩变量）
├── postcss.config.js
...
本 `SPEC.md` 为**产品与体验层唯一 SSOT**；视觉规格见 `DESIGN.md`。实现落点、目录与「当前代码与规格差距」以仓库源码为准；**实现层面的已知限制**见 [`AGENTS.md`](AGENTS.md) §10。

### 9.1 Build 模式：`all` / `any` 二元约束（不变量）
...
以下为**产品与教学设计**层面的禁忌。引擎与实现层面的已知限制（如 `after()` 模拟、`P2TR` 占位、移动端 Playground、`/compare` 未实现等）的完整清单见 [`AGENTS.md`](AGENTS.md) §10。

1. **不要一上来让用户写 Miniscript** -- 默认入口是场景卡片或 Policy 编辑器
2. **不要引入任何 LLM / 外部 AI API** -- 编译、分析、可视化、解释文字均为确定性实现，不调用外部 AI 服务
