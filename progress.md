...
### 2026-06-17：英文文案统一语气精修

- [x] **英文语气统一**：继续只调整 `src/lib/i18n/en.ts`，统一首页 CTA、导航状态词、钱包区块标题、资源页标题与若干产品说明句的语气与风格。
- [x] **验证**：`npm run lint` 通过，无 ESLint warnings / errors；`npm run typecheck` 通过。
- 未验证项：未运行 `npm run test:coverage`，因为本轮仍然只涉及静态英文文案。
- 下一步：如果继续，可再做一轮资源页与 FAQ 的术语一致性压缩，或停在当前版本。

### 2026-06-17：英文文案润色与检查

- [x] **英文 i18n 文案调整**：按翻译修改意见，仅修正 `src/lib/i18n/en.ts` 中的英文表述，未改动中文文案与业务逻辑。
- [x] **验证**：`npm run lint` 通过，无 ESLint warnings / errors；`npm run typecheck` 通过。
- 未验证项：未运行 `npm run test:coverage`，因为本次变更仅涉及英文静态文案。
- 下一步：如需继续，可按同样标准再做一轮全站英文文案的语气统一或资源页文案精修。
