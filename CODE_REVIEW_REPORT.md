...
7. **最后 — 文档与小清理**：P3。其中 P3-04（`t()` 单次替换）当前没有文案触发，**保留为低优先级**，可与下一次 i18n 改动一并处理。
## P0 Blocking
未发现阻断级问题。
## P1 Major
...
问题：`t()` 使用 `text.replace()`，同一个 `{param}` 多次出现时只替换第一处。
影响：未来文案如果重复引用同一个变量，会出现残留 `{param}`。
证据：
- `src/lib/i18n/context.tsx:82`
建议：改成 `replaceAll` 或正则全局替换，并补同参数出现两次的测试。**优先级说明**：当前 `zh.ts` / `en.ts` 没有任何文案触发该路径，落地价值低；保留为 P3，建议与下一次 i18n 文案改动顺手处理，不必单独排期。
